  ...
  return (
    <Card className="w-full max-w-md h-[45vh] bg-transparent rounded-[2rem] p-4 relative border-0">
      ...
      <div className="flex justify-between gap-4 mb-4">
        ...
        <div className={`space-y-[2vh] flex flex-col items-center w-[25%] h-full ${
          isCurrentTurn 
          ...
          {Array.from({ length: 4 }).map((_, index) => (
            <button
              ...
              className="w-full h-[20%] flex items-center justify-center bg-gray-800 rounded-2xl shadow-lg text-[3vh] disabled:opacity-50"
            >
              {items[index] ? ITEMS[items[index] as keyof typeof ITEMS].emoji : '\u00A0'}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4 flex flex-col items-center">
          <Button 
            ...
            className={`w-full h-32 bg-gray-800 hover:bg-gray-700 rounded-2xl shadow-lg text-2xl font-normal disabled:opacity-50 ${
              isCurrentTurn
        ...
        <div className={`space-y-[2vh] flex flex-col items-center w-[25%] h-full ${
          ...
          {Array.from({ length: 4 }).map((_, index) => (
            <button
              ...
              className="w-full h-[20%] flex items-center justify-center bg-gray-800 rounded-2xl shadow-lg text-[3vh] disabled:opacity-50"
            >
              {items[index + 4] ? ITEMS[items[index + 4] as keyof typeof ITEMS].emoji : '\u00A0'}
            </button>
          ))}
        </div>
      </div>
